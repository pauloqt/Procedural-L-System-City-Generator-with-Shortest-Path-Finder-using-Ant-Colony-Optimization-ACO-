...
rules_grid = {
    "X": [
        ...
        "F-[[X]+X]+F[+FX]-X",
        "F[X]F[+X]+F[-X]X",
        "F[+X][-X]F+F[X]+F[+FX]-X",  #rule for curved paths
        "FF[+X][+X]FF[-X][-X]",  #rule for intersections
        "F[X]+[X]+F-F",  #rule for branching paths
        "FX+F+FX-F-F"  #rule for creating loops
    ],
    "F": [
        "FF",
        "F[+F]F[-F]F",
        "F[+FF][-FF]F"  #rule for curved paths
    ],
}
...
def calculate_polygon_area(polygon): #A function that calculates the area of polygon using shoelace formula
    area = 0
...
def point_in_polygon(point, polygon): #A function that determines kung ang point is inside the polygon
    x, y = point
    n = len(polygon)
    inside = False
    p1x, p1y = polygon[0]
...
def connect_dead_end_nodes_angle(nodes, edges, surface):
    #Creates a set of all nodes and existing edges for quick lookup
    all_nodes = set(nodes)
    existing_edges = set(edges)

    #Creates a dictionary to store the connected nodes for each node
    connected_nodes = {node: set() for node in all_nodes}

    #Populates the connected_nodes dictionary
    for edge in edges:
        start, end = edge
...
def connect_dead_end_nodes(nodes, edges, surface):
    #Creates a set of all nodes for quick lookup and a mapping from nodes to their indices
    all_nodes = set(nodes)
    node_index_map = {node: i for i, node in enumerate(nodes)}

    #Creates a dictionary to keep track of connections for each node
    connected_nodes = {node: set() for node in all_nodes}

    #Creates a set of existing edges for quick lookup
    existing_edges = set(edges)

    #Populates the connected_nodes dictionary based on the existing edges
    for start, end in edges:
        if start in all_nodes and end in all_nodes:
            connected_nodes[start].add(end)
            connected_nodes[end].add(start)

    #Calculates the maximum possible distance on the surface
...
def draw_lsystem(sequence, step_size, surface):
    ...
    for char in sequence:
        ...
        if char == "F":  #Move forward
            ...
            pygame.draw.line(surface, GRAY, (x, y), (new_x, new_y), 7)

            nodes.append((new_x, new_y))  #Add the node to the node list
            edges.append(((x, y), (new_x, new_y)))
            x, y = new_x, new_y

        elif char == "+":  #Rotate the heading to the right
            current_angle += angle
        elif char == "-":  #Rotate the heading to the right
            current_angle -= angle
...
#Generate the string of L-system
def generate_lsystem(axiom, rules, max_segments):
    ...
    while True:
        ...
        for char in sequence:  #Loop through the sequence
            if char in rules:  #Check if the character ay X or F
                next_sequence += random.choice(rules[char])
            else: #if constant, ignore
                next_sequence += char
...
def heuristic(a, b): #returns the euclidean distance between 2 nodes
    return math.sqrt((a[0] - b[0]) ** 2 + (a[1] - b[1]) ** 2) #gamit euclidean formula to calculate the distance from one node to another given its coordinates

def astar(graph, start, end, nodes):

    if not graph or start == end:   #if the graph is empty or the start node is the same as the end node, return either the start node or None.
    ...
    f_score[start] = heuristic(nodes[start], nodes[end]) #initially ang f-score ay ang heuristic value nila)
